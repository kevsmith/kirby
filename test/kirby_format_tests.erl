...
seg_entry_v1_test_() ->
     [{"Create small entry",
       fun() -> Data = kirby_test_util:random_binary(32767),
                {ok, Entry} = kirby_format:make_seg_entry(Data),
      ...
      {"Parse truncated entry",
       fun() -> Data = kirby_test_util:random_binary(19777),
                {ok, Entry0} = kirby_format:make_seg_entry(Data),
                %% Truncate entry
...
seg_md_test_() ->
     [{"Create segment metadata",
       fun() -> Start = random:uniform(5000),
               ...
               <<SegIdx1:16/integer, ?SEG_VERSION:8/integer, Start1:64/integer, Size1:64/integer>> = Md,
